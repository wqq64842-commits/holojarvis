...
def web_search(query: str) -> str:
    ...
    subprocess.run(["open", f"https://www.bing.com/search?q={q}"])
...
def take_screenshot() -> str:
    path = datetime.datetime.now().strftime(
        "~/Desktop/截图-%Y%m%d-%H%M%S.png"
    )
    subprocess.run(["screencapture", os.path.expanduser(path)])
    return f"截图已保存到桌面"
...
def run_shell(command: str) -> str:
    """执行一条 shell 命令并返回输出（多步任务的万能手段）。

    危险/批量/删除类操作应先经用户确认；删除请用 move_to_trash 而非 rm。
    """
    try:
...
TOOL_SCHEMAS = [
    {
        "name": "open_app",
        "description": "打开一个 Mac 应用程序，例如 微信、Safari、备忘录、计算器、Music。",
        "input_schema": {
            "type": "object",
    ...
    {
        "name": "send_wechat",
        "description": "用 Mac 版微信给某个联系人发送一条文字消息。调用前务必已向用户口头确认『发给谁、发什么内容』。",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "run_shell",
        "description": "执行一条 macOS shell 命令并返回输出，用于多步任务（建文件夹、批量移动/重命名、查询等）。注意：删除文件请改用 move_to_trash；批量或有风险的操作请先向用户口头确认再执行。",
        "input_schema": {
            "type": "object",
